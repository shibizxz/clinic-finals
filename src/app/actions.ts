...
export async function createBookingAction(formData: FormData) {
  ...
  try {
    ...
    if (requestedTime) {
      params.set("time", requestedTime);
    }
    params.set("error", getActionErrorMessage(error));
    redirect(`/book/${doctorId}?${params.toString()}`);
  }
...
export async function requestRescheduleAction(formData: FormData) {
  ...
  try {
    ...
    params.set("error", getActionErrorMessage(error));
    redirect(`/dashboard/appointments/${appointmentId}?${params.toString()}`);
  }
