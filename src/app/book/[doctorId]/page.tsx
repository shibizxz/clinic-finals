...
export default async function BookDoctorPage({
  ...
  searchParams: Promise<{ date?: string; time?: string; error?: string }>;
}) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="rounded-[32px] border border-[var(--line)] bg-[var(--surface)] p-7 shadow-[var(--shadow)]">
        {query.error ? (
          <div className="mb-6 rounded-[24px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-medium text-rose-950">
            {query.error}
          </div>
        ) : null}
